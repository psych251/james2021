...
// Story condition (A/B/C) from URL: ?cond=A
...
// Session (day) from URL: ?session=1 or ?session=2
// session=1 → Day 1 (story + tests)
// session=2 → Day 2 (tests only)
const urlSession = new URLSearchParams(location.search).get('session');
const SESSION = (urlSession || '1');               // default to Day 1
...
const jsPsych = initJsPsych({
  on_finish: () => {
    ...
    // NOTE: useful for debugging; hide in production if you don't
    // want participants to see raw data.
...
// This function appends ALL test phases (Day 1 + Day 2) to the timeline.
// It assumes the following are defined in stem_completion.js:
//   prolificIdTrial, mic_intro, mic_check, mic_verification,
//   stem_overview, stem_instructions, makeStemCompPhase(), stem_finish.
//
// You can add other test tasks here later (e.g., forced choice, recognition).
function addTestPhases(timeline) {
  // Prolific ID entry
  timeline.push(prolificIdTrial);

  // Microphone checks
  timeline.push(mic_intro);
  timeline.push(mic_check);
  timeline.push(mic_verification);

  // Overview + stem completion task
  timeline.push(stem_overview);
  timeline.push(stem_instructions);
  timeline.push(...makeStemCompPhase());
  timeline.push(stem_finish);

  // TODO: Add other test tasks for this experiment here, e.g.:
  // timeline.push(...makeTestTaskA());
  // timeline.push(...makeTestTaskB());

  // Final generic outro (after all tests)
  timeline.push({
    ...
    stimulus: `
      <div style="text-align:center; padding:40px;">
        ...
        <p>You may now submit your completion code on Prolific.</p>
      </div>
    `,
    choices: ['Finish'],
    data: { event: 'final_outro', condition: CONDITION, session: SESSION },
    on_start: () => {
      const ended = performance.now();
      const duration = ended - exp_started_at_ms;

      console.log(
        `[${new Date().toISOString()}] END — session: ${SESSION}, condition: ${CONDITION}, duration_ms: ${Math.round(duration)}`
      );
...
// ---------- SESSION 2: tests-only (no story) ----------
if (SESSION === '2') {

  console.log(
    `[${new Date().toISOString()}] START — SESSION 2 (tests only), condition: ${CONDITION}`
  );

  addTestPhases(timeline);     // only test phases
  ...
  // ---------- SESSION 1: story + tests ----------

  // Intro screen for the story
  timeline.push({
    type: jsPsychHtmlButtonResponse,
    stimulus: `
    ...
    `,
    choices: ['Start'],
    data: { event: 'intro', condition: CONDITION, session: SESSION }
  });
  ...
  d3.csv(CSV_PATH).then((rows) => {
    ...
    storyRows.forEach((r, i) => {
      ...
      trials.push({
        ...
        on_start: () => {
          ...
          console.log(
            `[${new Date().toISOString()}] STORY TRIAL ${i + 1} — image: ${pic} | audio: ${audio}`
          );
        },
        data: {
          event: 'story_trial',
    ...
    timeline.push({
      ...
      data: { event: 'preload', condition: CONDITION, session: SESSION }
    });

    // Push story trials (in order; no randomization)
    timeline.push(...trials);

    // Short transition screen before test phases
    timeline.push({
      type: jsPsychHtmlButtonResponse,
      stimulus: `
        <div style="text-align:center; padding:40px;">
          <h2>The story is finished.</h2>
          <p>Next, we will ask you some questions and memory tasks about the story.</p>
        </div>
      `,
      choices: ['Continue'],
      data: { event: 'story_done', condition: CONDITION, session: SESSION }
    });

    // Add all test phases *after* the story
    addTestPhases(timeline);

    // ---- START LOGGING + RUN ----
    console.log(
      `[${new Date().toISOString()}] START — SESSION 1 (story + tests), condition: ${CONDITION}`
    );
    attachMonitors();
    isRunning = true;
